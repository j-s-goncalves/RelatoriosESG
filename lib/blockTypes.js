...
export const StructuredTableContentSchema = withConfidentiality({
  ...
  meta: z.record(z.string(), z.union([z.string(), z.null()])).default({}),
});
...
export const RepeatableTableContentSchema = withConfidentiality({
  ...
  // Campo de referência/URL — alternativa ao preenchimento directo (ex. B4)
  reference_url: z.string().default(""),
});
...
export const NarrativeContentSchema = withConfidentiality({
...
const CONTENT_SCHEMAS = {
  ...
  [BLOCK_TYPES.NARRATIVE]:                    NarrativeContentSchema,
};
...
export function emptyContent(definition) {
  ...
  switch (block_type) {
    ...
    case BLOCK_TYPES.NARRATIVE:
      return NarrativeContentSchema.parse({});
