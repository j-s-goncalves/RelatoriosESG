...
export async function upsertBlock(undertakingId, periodId, questionnaireCode, content) {
  const sql = getSql();
  await sql`
    INSERT INTO blocks (block_type, questionnaire_code, undertaking_id, period_id, content, state, provenance, updated_at)
    VALUES ('mini_questionnaire', ${questionnaireCode}, ${undertakingId}, ${periodId}, ${JSON.stringify(content)}, 'elaboracao', 'novo', now())
