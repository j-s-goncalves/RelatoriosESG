...
const DISCLOSURES = [
  ...
  // Estrutura exigida pelo §32: 2 linhas × 3 colunas (renovável | não renovável | total)
  {
    ...
    rows: [
      { row_code: "elec",  label: "Electricidade" },
      { row_code: "fuels", label: "Combustíveis" },
    ],
    columns: [
      { col_code: "renewable_mwh",    label: "Renovável",     value_type: "numeric", unit: "MWh", default_value: null },
      { col_code: "nonrenewable_mwh", label: "Não renovável", value_type: "numeric", unit: "MWh", default_value: null },
      { col_code: "total_mwh",        label: "Total",         value_type: "numeric", unit: "MWh", default_value: null, computed: true },
    ],
  },
  ...
  // §34: se a informação estiver disponível publicamente, pode ser substituída por referência/URL.
  {
    code: "B4_POLLUTION",
    block_type: BLOCK_TYPES.REPEATABLE_TABLE_WITH_TOTALS,
    title: "VSME B4 — Poluição",
    short_label: "B4 — Poluição",
    url_field_label: "Referência/URL (alternativa ao preenchimento directo da tabela)",
    columns: [
  ...
  // Estrutura exigida pelo §39(a)(b): fluxo, perigoso, unidade, desviado/eliminado, total
  {
    code: "B7_WASTE",
    block_type: BLOCK_TYPES.REPEATABLE_TABLE_WITH_TOTALS,
    ...
    columns: [
      { col_code: "waste_type", label: "Fluxo de resíduo",                          value_type: "text",    unit: null, default_value: "" },
      { col_code: "hazardous",  label: "Perigoso/Não perigoso",                     value_type: "text",    unit: null, default_value: "Não perigoso" },
      { col_code: "unit",       label: "Unidade",                                   value_type: "text",    unit: null, default_value: "ton" },
      { col_code: "recycled",   label: "Desviado para reciclagem/reutilização",     value_type: "numeric", unit: null, default_value: null },
      { col_code: "disposed",   label: "Encaminhado para eliminação",               value_type: "numeric", unit: null, default_value: null },
      { col_code: "total",      label: "Total",                                     value_type: "numeric", unit: null, default_value: null, computed: true },
    ],
  },

  // ── B7_WASTE_TOTALS: Quadro de totais de resíduos (StructuredTable) ──────────
  // Exigido pelo §39(a)(b): 3 categorias × massa (ton) e volume (m³)
  {
    code: "B7_WASTE_TOTALS",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "VSME B7 — Totais de Resíduos por Categoria",
    short_label: "B7 — Totais Resíduos",
    rows: [
      { row_code: "hazardous",    label: "Resíduos perigosos" },
      { row_code: "nonhazardous", label: "Resíduos não perigosos" },
      { row_code: "all",          label: "Total geral" },
    ],
    columns: [
      { col_code: "mass_ton", label: "Massa",   value_type: "numeric", unit: "ton", default_value: null },
      { col_code: "vol_m3",   label: "Volume",  value_type: "numeric", unit: "m³",  default_value: null },
    ],
  },

  // ── B7_MATERIALS: Materiais — campo gate (MiniQuestionnaire) ─────────────────
  // §39(c): gate "opera em sector com fluxos de materiais significativos?"
  {
    code: "B7_MATERIALS",
    block_type: BLOCK_TYPES.MINI_QUESTIONNAIRE,
    title: "VSME B7 — Materiais (Aplicabilidade)",
    short_label: "B7 — Materiais",
    items: [
      {
        node_type: "item",
        item_code: "significant_material_flows",
        label: "A empresa opera num sector com fluxos de materiais significativos (consumo relevante de matérias-primas ou materiais com impacto ambiental)?",
        value_type: "boolean",
  ...
  // ── B7_MATERIALS_DETAIL: Materiais nomeados individualmente (RepeatableTable) ─
  // Preencher apenas se B7_MATERIALS gate = Sim
  {
    code: "B7_MATERIALS_DETAIL",
    block_type: BLOCK_TYPES.REPEATABLE_TABLE,
    title: "VSME B7 — Materiais (Detalhe por material-chave)",
    short_label: "B7 — Materiais (detalhe)",
    columns: [
      { col_code: "material_name", label: "Designação do material", value_type: "text",    unit: null, default_value: "" },
      { col_code: "quantity",      label: "Quantidade",              value_type: "numeric", unit: null, default_value: null },
      { col_code: "unit",          label: "Unidade",                 value_type: "text",    unit: null, default_value: "ton" },
    ],
  },
  ...
  // ── B8_GENDER: Trabalhadores por género (StructuredTable) ────────────────────
  // §40: nº de empregados por género
  {
    code: "B8_GENDER",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "VSME B8 — Trabalhadores por Género",
    short_label: "B8 — Género",
    rows: [
  ...
  // ── B8_COUNTRIES: Trabalhadores por país (RepeatableTable) ───────────────────
  // §40: nº de empregados por país onde a empresa tem operações significativas
  {
    code: "B8_COUNTRIES",
    block_type: BLOCK_TYPES.REPEATABLE_TABLE,
    title: "VSME B8 — Trabalhadores por País",
    short_label: "B8 — Países",
    columns: [
      { col_code: "country",   label: "País",         value_type: "text",    unit: null, default_value: "" },
      { col_code: "headcount", label: "Efectivo",     value_type: "numeric", unit: null, default_value: null },
      { col_code: "pct_total", label: "% do total",   value_type: "numeric", unit: "%",  default_value: null },
    ],
  },

  // ── B9_DIVERSITY: Diversidade etária e de género (informação adicional) ───────
  // Não tem correspondência directa numa disclosure do Módulo Básico — mantido como
  // informação adicional voluntária.
  {
    code: "B9_DIVERSITY",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "B9 — Diversidade Etária e de Género (informação adicional)",
    short_label: "B9 — Diversidade (adicional)",
    rows: [
  ...
  // ── B9_SAFETY: Acidentes e lesões laborais (StructuredTable) ─────────────────
  // §41 (B9 na numeração correcta do standard): acidentes de trabalho registáveis e fatalidades
  {
    code: "B9_SAFETY",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "VSME B9 — Saúde e Segurança: Acidentes e Lesões",
    short_label: "B9 — Acidentes",
    rows: [
  ...
  // ── B10_HEALTH_POLICY: Política de SST (informação adicional) ────────────────
  // Não tem correspondência directa numa disclosure do Módulo Básico — mantido como
  // informação adicional voluntária.
  {
    code: "B10_HEALTH_POLICY",
    block_type: BLOCK_TYPES.MINI_QUESTIONNAIRE,
    title: "B10 — Política de Saúde e Segurança no Trabalho (informação adicional)",
    short_label: "B10 — Política SST (adicional)",
    items: [
      {
        node_type: "item",
  ...
  // ── B10_REMUNERATION: Remuneração e diferença salarial por género (StructuredTable) ─
  // §42: gap salarial de género (remuneração mediana homens vs mulheres)
  {
    code: "B10_REMUNERATION",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "VSME B10 — Remuneração e Diferença Salarial por Género",
    short_label: "B10 — Remuneração",
    rows: [
      { row_code: "men",   label: "Homens" },
      { row_code: "women", label: "Mulheres" },
    ],
  ...
  // ── B10_CONDITIONS: Condições de emprego — métricas adicionais (MiniQuestionnaire) ─
  // §42: salário mínimo, % acordo colectivo, horas de formação
  {
    code: "B10_CONDITIONS",
    block_type: BLOCK_TYPES.MINI_QUESTIONNAIRE,
    title: "VSME B10 — Condições de Emprego",
    short_label: "B10 — Condições",
    items: [
      {
        node_type: "item",
        item_code: "min_wage_eur",
        label: "Salário mínimo aplicável (EUR/ano)",
        value_type: "numeric",
        unit: "EUR",
        allows_specify: false,
        default_value: null,
      ...
      {
        node_type: "item",
        item_code: "pct_collective_agreement",
        label: "% de trabalhadores cobertos por acordo colectivo de trabalho",
        value_type: "numeric",
        unit: "%",
        allows_specify: false,
        default_value: null,
      ...
      {
        node_type: "item",
        item_code: "avg_training_hours",
        label: "Horas médias de formação por trabalhador/ano",
        value_type: "numeric",
        unit: "horas",
        allows_specify: false,
        default_value: null,
  ...
  // ── C5_TURNOVER: Rotatividade — admissões e saídas (StructuredTable) ──────────
  // §58 (Módulo Abrangente): indicador de rotatividade voluntário para o Módulo Básico
  // Movido de B9 conforme numeração correcta do standard.
  {
    code: "C5_TURNOVER",
    block_type: BLOCK_TYPES.STRUCTURED_TABLE,
    title: "C5 — Rotatividade: Admissões e Saídas",
    short_label: "C5 — Rotatividade",
    rows: [
